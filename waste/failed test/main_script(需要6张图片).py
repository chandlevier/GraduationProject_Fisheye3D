...
def loadImages(pointer):
    ...
    for i in tqdm(range(WINDOW_SIZE)):  # Tqdm：快速,可扩展的Python进度条,可以在 Python 长循环中添加一个进度提示信息
        ...
        front = cv2.imread('./input/front/' + zero_string +
                            str(pointer + i) +
                            '.png')
        left = cv2.imread('./input/left/' + zero_string +
                            str(pointer + i) +
                            '.png')
        right = cv2.imread('./input/right/' + zero_string +
                            str(pointer + i) +
                            '.png')
        top = cv2.imread('./input/top/' + zero_string +
                            str(pointer + i) +
                            '.png')
        bottom = cv2.imread('./input/bottom/' + zero_string +
                            str(pointer + i) +
                            '.png')
        back = cv2.imread('./input/back/' + zero_string +
                            str(pointer + i) +
                            '.png')
        # 将6张图片拼接成立方体贴图的样子，并保存至output_image变量中，加入cubemap数组中
