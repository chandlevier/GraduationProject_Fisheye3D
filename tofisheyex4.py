...
def loadImages(pointer, input_file):
    """
    ...
    for i in tqdm(range(WINDOW_SIZE)):  # Tqdm：快速,可扩展的Python进度条,可以在 Python 长循环中添加一个进度提示信息
        ...
        if i + pointer < 10:
            ...
            zero_string = 0*'0'
        #-------------------- 每个鱼眼相机修改一次 --------------------#
        front = cv2.imread(input_file + '/front/' + zero_string +
                            str(pointer + i) +
                            '.png')
        left = cv2.imread(input_file + '/left/' + zero_string +
                            str(pointer + i) +
                            '.png')
        right = cv2.imread(input_file + '/right/' + zero_string +
                            str(pointer + i) +
                            '.png')
        top = cv2.imread(input_file + '/top/' + zero_string +
                            str(pointer + i) +
                            '.png')
        bottom = cv2.imread(input_file + '/bottom/' + zero_string +
                            str(pointer + i) +
                            '.png')
        #-------------------- 每个鱼眼相机修改一次 --------------------#
...
def main():
    ...
    main function
    """
    # input_file ./input/fishf或./input/fishb或./input/fishr或./input/fishl
    parser = ArgumentParser()
    parser.add_argument('input_file', help='input files')
    args = parser.parse_args()
    cubemap_to_fisheye(args.input_file)
    
    elapsed_time = time.time() - start_time
    
    print('\nElapsed time: ', elapsed_time ,' seconds')

def cubemap_to_fisheye(input_file):
    ...
    while number_of_frames < NUMBER_OF_FRAMES:
        cubemap = loadImages(pointer, input_file)
