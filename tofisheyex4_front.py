...
def loadImages(pointer):
    ...
    for i in tqdm(range(WINDOW_SIZE)):  # Tqdm：快速,可扩展的Python进度条,可以在 Python 长循环中添加一个进度提示信息
        ...
        if i + pointer < 10:
            ...
            zero_string = 0*'0'
        #-------------------- 每个鱼眼相机修改一次 --------------------#
        front = cv2.imread('./input/fishf/front/' + zero_string +
                            str(pointer + i) +
                            '.png')
        left = cv2.imread('./input/fishf/left/' + zero_string +
                            str(pointer + i) +
                            '.png')
        right = cv2.imread('./input/fishf/right/' + zero_string +
                            str(pointer + i) +
                            '.png')
        top = cv2.imread('./input/fishf/top/' + zero_string +
                            str(pointer + i) +
                            '.png')
        bottom = cv2.imread('./input/fishf/bottom/' + zero_string +
                            str(pointer + i) +
                            '.png')
        #-------------------- 每个鱼眼相机修改一次 --------------------#
