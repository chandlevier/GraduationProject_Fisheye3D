# 在车上安装5个rgb相机，然后存储5个相机图像，后续将其拼接成鱼眼图像
...
parser.add_argument('--save-path',            default='/home/piaozx/文档/carla-code/cube2test/input/', help='Synchronous mode execution')
# parser.add_argument('-w', '--number-of-walkers',  metavar='W',       default=200,              type=int, help='number of walkers (default: 50)')
# parser.add_argument('--filterw',                  metavar='PATTERN', default='walker.pedestrian.*',      help='pedestrians filter (default: "walker.pedestrian.*")')
...
def main(args):
    ...
    try:
        ...
        # 手动规定
        transform_vehicle = carla.Transform(carla.Location(x=-1, y=-15, z=0.3), carla.Rotation(0, -90, 0))
        # 随机选择
        ...
        # Transform(Location(x=32.176502, y=-177.343231, z=0.300000)
        # Rotation(pitch=0.000000, yaw=91.532082, roll=0.000000))
        ...
        cam_location = carla.Location(x=2.347, z=0.7)
        # cam_location = carla.Location(x=0.65, y=1.1, z=1)
        # cam_bp.set_attribute('sensor_tick', '0.1')
        # 调用库函数，设置5个相机
        camera_script.rgbcams_buildup(world, cam_bp, cam_location, ego_vehicle, sensor_list, sensor_queue, yaw0=0)
        ...
        #--------------------------- 其他npc部分 ---------------------------#
        npc.spawn_actors(world, blueprint_library, transform_vehicle, actor_list)
        walker1, walker2 = npc.spawn_walker(world, blueprint_library, transform_vehicle, actor_list)
            ...
            try:
                ...
                for i in range(0, len(actor_list)):
                    ...
                    if id[0] == 'v':
                        ...
                        l = 1
                        w = 1
                    camera_script.create_bevimage(image, x, -y, yaw, l, w, w_frame, i)
                    # print("The actor%d %s is located at(x=%.4f, y=%.4f, z=%.4f), yaw=%.4f" % ((i+1), id, x, y, z, yaw))
        ...
        for actor in actor_list:
            if actor:
                print("Destroy actor %s" % actor.type_id)
                actor.destroy()
