...
def show_multi_modality_result(bev_file, frame, img,
                               gt_bboxes,
                               pred_bboxes,
                               proj_mat,
                               out_dir,
    ...
    # 修改预测结果存储路径result_path
    # 原先程序设置存储路径为“指定文件夹”+“每个图像各自的文件夹”
    # result_path = osp.join(out_dir, filename)
    result_path = out_dir
    mmcv.mkdir_or_exist(result_path)
    ...
    if gt_bboxes is not None:
        gt_img = draw_bbox(
            gt_bboxes, img, proj_mat, bev_file, frame, img_metas, color=gt_bbox_color)
        mmcv.imwrite(gt_img, osp.join(result_path, f'{filename}_gt.png'))

    if pred_bboxes is not None:
        pred_img = draw_bbox(
            pred_bboxes, img, proj_mat, bev_file, frame, img_metas, color=pred_bbox_color)
