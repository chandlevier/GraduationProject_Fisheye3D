...
def fisheye_stitch():
    ...
    images = glob.glob("output/fishf/*.png")
    # 创建文件夹
    camera_script.mkdir_folder("/home/piaozx/文档/carla-code/carlafisheye", "outcome", None)
    for i in tqdm(range(len(images))):  # Tqdm：快速,可扩展的Python进度条,可以在 Python 长循环中添加一个进度提示信息
